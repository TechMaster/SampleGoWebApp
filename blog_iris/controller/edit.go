...
func Edited(ctx iris.Context) {
	...
	time := time.Now().Local().Format("2006-01-02 10:20:58")
	ctx.ViewData("Created_at", time)
	...
	i, _ := strconv.Atoi(temp)
	id := int16(i)
	model.Update(db, post.Title, post.Alias, post.Intro_text, post.Full_text, post.Image, post.Published, post.Categories,  time, id)
}